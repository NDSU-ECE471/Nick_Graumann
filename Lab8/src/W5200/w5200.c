...
static uint32 SBUFBASEADDRESS[MAX_SOCK_NUM]; /**< Tx buffer base address by each channel */
static uint32 RBUFBASEADDRESS[MAX_SOCK_NUM]; /**< Rx buffer base address by each channel */
static uint8 SUBNET[4];

void GPIO_Configuration(void)
{
	LPC_GPIO0 ->FIODIR |= (1<<21) +(1<<22) + (1<<16); //21 -> RST, 22 -> PWDN, 16 -> nCS
	LPC_GPIO0 -> FIOSET = (1<<21); // RST HIGH
	LPC_GPIO0 -> FIOCLR = (1<<22); //PWDN LOW
