...
const uint8_t StringDescriptor[] =
{
	...
	USB_STRING_LEN(26), /* ProductString */
	DTYPE_String,
	WBVAL('L'),WBVAL('P'),WBVAL('C'),WBVAL('U'),WBVAL('S'),WBVAL('B'),WBVAL('l'),WBVAL('i'),WBVAL('b'),WBVAL(' '),
	WBVAL('G'),WBVAL('e'),WBVAL('n'),WBVAL('e'),WBVAL('r'),WBVAL('i'),WBVAL('c'),WBVAL(' '),WBVAL('H'),WBVAL('I'),WBVAL('D'),WBVAL(' '),
...
extern const USB_Descriptor_HIDReport_Datatype_t PROGMEM GenericReport[];
...
{
	return Generic_HID_Interface.Config.ReportINEndpointNumber;
}
...
{
	*dest = (uint8_t*)GenericReport;
	return ConfigurationDescriptor.HID_GenericHID.HIDReportLength;
}
uint32_t CALLBACK_UsbdHid_Register_ReportInBuffer(uint8_t **dest)
{
	*dest = (uint8_t*)Generic_HID_Interface.Config.PrevReportINBuffer;
	return (uint32_t)Generic_HID_Interface.Config.PrevReportINBufferSize;
}
void CALLBACK_UsbdHid_SetReport(uint8_t **reportoutbuffer, uint32_t reportoutsize)
{
	CALLBACK_HID_Device_ProcessHIDReport(NULL, 0, 0, *reportoutbuffer, reportoutsize);
