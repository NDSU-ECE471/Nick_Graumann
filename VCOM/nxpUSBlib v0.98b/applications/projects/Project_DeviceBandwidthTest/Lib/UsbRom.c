...
typedef ATTR_IAR_PACKED struct _st_Inquiry_String {
	uint8_t  VendorID[8];
...
const USB_Descriptor_DeviceQualifier_t DeviceQualifierDescriptor =
{
...
const uint8_t StringDescriptor[] =
{
	...
	WBVAL('M'),WBVAL('a'),WBVAL('s'),WBVAL('s'),WBVAL(' '),WBVAL('S'),WBVAL('t'),WBVAL('o'),WBVAL('r'),WBVAL('a'),WBVAL('g'),WBVAL('e'),WBVAL(' '),
...
void translate_rd( uint32_t offset, uint8_t** buff_adr, uint32_t length)
{
	MassStorage_Read(offset, *buff_adr, length);
}

void translate_wr( uint32_t offset, uint8_t** buff_adr, uint32_t length);
void translate_wr( uint32_t offset, uint8_t** buff_adr, uint32_t length)
{
	MassStorage_Write(offset, *buff_adr, length);
}
...
ErrorCode_t translate_verify( uint32_t offset, uint8_t* src, uint32_t length)
{
  if (MassStorage_Verify((void*)offset, src, length))
	return LPC_OK;
